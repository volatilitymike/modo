...
# ─────────────────────────────────────────────────────────
# **Function to Calculate Standard Deviations of Change (Fix)**
def calculate_sd_of_change_cc(data, window=13):
    """
    Calculate daily price changes measured in standard deviations (Close-to-Close).

    Parameters:
        data (pd.DataFrame): DataFrame containing at least the 'Close' column.
        window (int): The window size for rolling standard deviation. Default is 13.

    Returns:
        pd.DataFrame: DataFrame with additional columns for Log Change, Rolling Std Dev, and SD of Change CC.
    """
    # Ensure data is sorted by date
    data = data.copy()  # Ensure modification happens on a copy to prevent warnings

    # Calculate logarithmic daily changes
    data['Log_Change_CC'] = np.log(data['Close'] / data['Close'].shift(1))

    # Rolling standard deviation (13-period window)
    ...
    # Optional: Handle infinite or undefined values if any
...
if st.sidebar.button("Fetch Data"):
    try:
        ...
        # 1) Asegura que las columnas NO sean MultiIndex
        if isinstance(intraday.columns, pd.MultiIndex):
            # Aplana el nivel superior si es que lo hay
            intraday.columns = intraday.columns.map(lambda x: x[0] if isinstance(x, tuple) else x)

        # 2) Convierte el índice en columna Date
        intraday.reset_index(inplace=True)
        intraday.rename(columns={"index": "Date"}, inplace=True)  # Asegurar que la columna de fecha tenga nombre correcto

        # 3) Asegura que el nombre sea Date
        if "Date" not in intraday.columns:
            intraday.rename(columns={intraday.columns[0]: "Date"}, inplace=True)

        # 4) Verifica que todas las columnas necesarias estén presentes
        required_columns = ["Date", "Open", "High", "Low", "Close", "Volume"]
        missing = [col for col in required_columns if col not in intraday.columns]
        if missing:
            st.error(f"Missing required columns: {missing}")
            st.stop()
        intraday = intraday[required_columns]

        # ─────────────────────────────────────────────────────────
        # Crea las columnas adicionales
      # Definir el período para el promedio móvil (por ejemplo, 50 períodos)
        period = 52
        ...
        # **Add the SD of Change Calculation**
        ...
        def calculate_td_countdown(data):
            ...
            # Find where Buy/Sell Setups are completed
            buy_setup_completed_indices = data.index[data['Buy Setup'] == "Buy Setup Completed"].tolist()
            sell_setup_completed_indices = data.index[data['Sell Setup'] == "Sell Setup Completed"].tolist()
            ...
            for i in range(len(data)):
                # Activate Buy Countdown when Buy Setup is completed
                if i in buy_setup_completed_indices:
                    buy_countdown_active = True
                    sell_countdown_active = False  # Reset opposing countdown
                    buy_count = 0  # Start at 0 (Corrected)
                    continue

                # Activate Sell Countdown when Sell Setup is completed
                if i in sell_setup_completed_indices:
                    sell_countdown_active = True
                    buy_countdown_active = False  # Reset opposing countdown
                    sell_count = 0  # Start at 0 (Corrected)
                    continue
        ...
        columns_to_show = ["Date","" "Close", "RV","SD_of_Change_CC","Dollar Change","Pct Change",'Kijun_Alert','Buy Setup','Sell Setup','Setup Qualifier','Buy Countdown','Sell Countdown','Buy Combo Countdown','Sell Combo Countdown','TD Pressure',"TD Pressure Alert","TD REI","TD REI Alert","TD POQ Signal","Alert"]
        st.dataframe(intraday[columns_to_show], use_container_width=True)
