...
# 3) Now that intraday is fully processed,
                ...
                def mark_exit_after_call_alert(intraday):
                    """Track F% after Entry_Alert_Long and return the index of first Rainy cloud (TD Demand break) for exit."""
                    intraday["Exit_Alert_Long"] = False  # Initialize

                    if "Entry_Alert_Long" not in intraday.columns or "F_numeric" not in intraday.columns or "TD Demand Line F" not in intraday.columns:
                        return intraday

                    entry_indices = intraday.index[intraday["Entry_Alert_Long"]]

                    for entry_idx in entry_indices:
                        start_idx = intraday.index.get_loc(entry_idx)
                        for i in range(start_idx + 1, len(intraday)):
                            row = intraday.iloc[i]
                            if pd.notna(row["F_numeric"]) and pd.notna(row["TD Demand Line F"]):
                                if row["F_numeric"] < row["TD Demand Line F"]:
                                    intraday.at[intraday.index[i], "Exit_Alert_Long"] = True
                                    break  # Stop after first rainy break
                    return intraday


                intraday = mark_exit_after_call_alert(intraday)

                def mark_exit_after_put_alert(intraday):
                    """Track F% after Entry_Alert_Short and return the index of first Sunny cloud (TD Supply break) for exit."""
                    intraday["Exit_Alert_Short"] = False  # Initialize

                    if "Entry_Alert_Short" not in intraday.columns or "F_numeric" not in intraday.columns or "TD Supply Line F" not in intraday.columns:
                        return intraday

                    entry_indices = intraday.index[intraday["Entry_Alert_Short"]]

                    for entry_idx in entry_indices:
                        start_idx = intraday.index.get_loc(entry_idx)
                        for i in range(start_idx + 1, len(intraday)):
                            row = intraday.iloc[i]
                            if pd.notna(row["F_numeric"]) and pd.notna(row["TD Supply Line F"]):
                                if row["F_numeric"] > row["TD Supply Line F"]:
                                    intraday.at[intraday.index[i], "Exit_Alert_Short"] = True
                                    break  # Stop after first sunny break
                    return intraday
                intraday = mark_exit_after_put_alert(intraday)
 ...
 # 🟢   SPAN A & SPAN B
                    ...
                    intraday["Chikou_Emoji"] = np.where(intraday["Chikou_Position"] == "above", "🚦",
 ...
 # #🟢 Velocity
                   ...
                   # 🐆 Speed spike (Cheetah)
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        y=intraday.loc[mask_cheetah, "F_numeric"]+ 8,
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        y=intraday.loc[mask_turtle, "F_numeric"] - 8,
...
#🟢 TD DEMAND
               ...
               # Extract only the rows where TDST just formed
                    ...
                    # # 🚀 Breakout after ✈️ logic
                    # rocket_points = intraday[intraday["Breakout_Emoji"] == "🚀"]

                    # scatter_rocket = go.Scatter(
                    #     x=rocket_points["Time"],
                    #     y=rocket_points["F_numeric"] + 144,  # Add vertical offset so it's above price
                    #     mode="text",
                    #     text=rocket_points["Breakout_Emoji"],
                    #     textposition="top center",
                    #     name="Breakout 🚀",
                    #     textfont=dict(size=55),
                    #     showlegend=False
                    ...
                    # fig.add_trace(scatter_rocket, row=1, col=1)
                    ...
                    # Upward cross (🚣🏽) — Recovery from low
                    y_low_boat = go.Scatter(
                        x=intraday.loc[intraday["Y_Low_Cross"] == "🚣🏽", "Time"],
                        y=intraday.loc[intraday["Y_Low_Cross"] == "🚣🏽", "F_numeric"] + y_offset_up,
                        mode="text",
                        textfont=dict(size=55),
                        text=["🚣🏽"] * len(intraday.loc[intraday["Y_Low_Cross"] == "🚣🏽"]),
                        textposition="top center",
                        name="Above Yesterday Low",
                        showlegend=False
                    )

                    # Downward cross (🛟) — Breach of low
                    y_low_rescue = go.Scatter(
                        x=intraday.loc[intraday["Y_Low_Cross"] == "🛟", "Time"],
                        y=intraday.loc[intraday["Y_Low_Cross"] == "🛟", "F_numeric"] + y_offset_down,
                        mode="text",
                        text=["🛟"] * len(intraday.loc[intraday["Y_Low_Cross"] == "🛟"]),
                        textposition="bottom center",
                        textfont=dict(size=55),
                        name="Below Yesterday Low",
                        showlegend=False
                    )

                    # Add both traces to the F% plot
                    fig.add_trace(y_low_boat, row=1, col=1)
                    fig.add_trace(y_low_rescue, row=1, col=1)
                    swimmer_points = intraday[intraday["Swimmer_Emoji"] == "🏊🏽‍♂️"]

                    scatter_swimmer = go.Scatter(
                        x=swimmer_points["Time"],
                        y=swimmer_points["F_numeric"] - 188,  # Offset down
                        mode="text",
                        text=swimmer_points["Swimmer_Emoji"],
                        textposition="bottom center",
                        name="New Lows 🏊🏽‍♂️",
                        textfont=dict(size=34),
                        showlegend=False
                    ...
                    # 🏎️ — Crossed above yesterday's close (0)
                    y_close_race = go.Scatter(
                        x=intraday.loc[intraday["Y_Close_Cross"] == "🏎️", "Time"],
                        y=intraday.loc[intraday["Y_Close_Cross"] == "🏎️", "F_numeric"] + y_offset_above,
                        mode="text",
                        textfont=dict(size=34),
                        text=["🏎️"] * len(intraday.loc[intraday["Y_Close_Cross"] == "🏎️"]),
                        textposition="top center",
                        name="Above Yesterday Close",
                        showlegend=False
                    )

                            # 🚵‍♂️ — Crossed below yesterday's close (0)
                    y_close_bike = go.Scatter(
                        x=intraday.loc[intraday["Y_Close_Cross"] == "🚵‍♂️", "Time"],  # <-- 🔥 FIXED
                        y=intraday.loc[intraday["Y_Close_Cross"] == "🚵‍♂️", "F_numeric"] + y_offset_below,
                        mode="text",
                        text=["🚵‍♂️"] * len(intraday.loc[intraday["Y_Close_Cross"] == "🚵‍♂️"]),
                        textposition="bottom center",
                        name="Below Yesterday Close",
                        textfont=dict(size=34),
                        showlegend=False
                    )
                    ...
                    # # Add to F% plot
                    # fig.add_trace(y_close_race, row=1, col=1)
                    # fig.add_trace(y_close_bike, row=1, col=1)
                    ...
                    # 🚶🏾 — Crossed above yesterday's open
                    y_open_walk = go.Scatter(
                        x=intraday.loc[intraday["Y_Open_Cross"] == "🚶🏾", "Time"],
                        y=intraday.loc[intraday["Y_Open_Cross"] == "🚶🏾", "F_numeric"] + y_offset_above,
                        mode="text",
                        text=["🚶🏾"] * len(intraday.loc[intraday["Y_Open_Cross"] == "🚶🏾"]),
                        textposition="top center",
                            textfont=dict(size=89),
                        name="Above Yesterday Open",
                        showlegend=False
                    )

                    # 🏃🏾 — Crossed below yesterday's open
                    y_open_run = go.Scatter(
                        x=intraday.loc[intraday["Y_Open_Cross"] == "🏃🏾", "Time"],
                        y=intraday.loc[intraday["Y_Open_Cross"] == "🏃🏾", "F_numeric"] + y_offset_below,
                        mode="text",
                        text=["🏃🏾"] * len(intraday.loc[intraday["Y_Open_Cross"] == "🏃🏾"]),
                        textposition="bottom center",
                            textfont=dict(size=89),
                        name="Below Yesterday Open",
                        showlegend=False
                    )

                    # Add to F% plot
                    fig.add_trace(y_open_walk, row=1, col=1)
                    fig.add_trace(y_open_run, row=1, col=1)
                ...
                short_entry_trace = go.Scatter(
                    ...
                    mode="text",
                    text=["Put Here ✅"] * intraday["Entry_Alert_Short"].sum(),
                    textposition="bottom center",
                ...
                long_entry_trace = go.Scatter(
                    ...
                    mode="text",
                    text=["Call Here ✅"] * intraday["Entry_Alert_Long"].sum(),
                    textposition="top center",
                    textfont=dict(size=21, color="lime"),
                    name="Long Entry (✅)"
                )
                ...
                fig.add_trace(scatter_long_delayed, row=1, col=1)
                fig.add_trace(scatter_short_delayed, row=1, col=1)




                exit_long_trace = go.Scatter(
                    x=intraday.loc[intraday["Exit_Alert_Long"], "Time"],
                    y=intraday.loc[intraday["Exit_Alert_Long"], "F_numeric"] - 10,
                    mode="text",
                    text=["Exit Call ⛅"] * intraday["Exit_Alert_Long"].sum(),
                    textposition="bottom center",
                    textfont=dict(size=18, color="red"),
                    name="Exit Call (⛅)"
                )
                fig.add_trace(exit_long_trace, row=1, col=1)


                exit_short_trace = go.Scatter(
                    x=intraday.loc[intraday["Exit_Alert_Short"], "Time"],
                    y=intraday.loc[intraday["Exit_Alert_Short"], "F_numeric"] + 10,
                    mode="text",
                    text=["Exit Put ☀️"] * intraday["Exit_Alert_Short"].sum(),
                    textposition="top center",
                    textfont=dict(size=18, color="red"),
                    name="Exit Put (☀️)"
                )
                fig.add_trace(exit_short_trace, row=1, col=1)
