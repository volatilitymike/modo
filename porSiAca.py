...
# 3) Now that intraday is fully processed,
                ...
                snapshot_cols = ["Time", "Close","Opening Price Signal","OPS Transition","Net Price","Net Price Direction","Bounce","Retrace",
                       "RVOL_5", "Day Type", "High of Day",
                        "Low of Day", "F%","TD Open","TD Trap","TD CLoP", "40ish","Tenkan_Kijun_Cross",
                 "CTOD Alert","Alert_Kijun","Alert_Mid","Wealth Signal"]  # Adjust as needed
                ...
                with ticker_tabs[0]:
                    ...
                    fig = make_subplots(
                        ...
                        vertical_spacing=0.1,
                         subplot_titles=("F% Chart", "Bounce %", "Retrace %"),
                        row_heights=[2, 0.5, 0.5]  # 👈 60% for F%, 20% each for the rest
                    ...
                    fig.add_trace(bounce_trace, row=2, col=1)

                    # Retrace plot (row 3)
                    retrace_trace = go.Scatter(
                        x=intraday["Time"],
                        y=intraday["Retrace"],
                        mode="lines",
                        name="Retrace %",
                    ...
                    fig.add_trace(retrace_trace, row=3, col=1)\
           ...
           # 🟢 Wealth Buy & Sell Signals (Plotted with RVOL Color Code)
                  ...
                  # (D) TD Trap Arrows - Only First Sell TD Trap
                    ...
                    #     name="F% MA (8)"
                    # )
                    # fig.add_trace(f_ma_8_line, row=1, col=1)



                    fig.update_yaxes(title_text="F%", row=1, col=1)
                    fig.update_yaxes(title_text="Bounce %", row=2, col=1)
                    fig.update_yaxes(title_text="Retrace %", row=3, col=1)
