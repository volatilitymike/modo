...
if st.sidebar.button("Run Analysis"):
   ...
   # Step 1: Calculate OBV
                ...
                # def add_bbw_honey_after_bee(df, bee_col="BBW_Tight_Emoji", honey_col="BBW_Honey_Emoji"):
                #     df[honey_col] = ""

                #     # Find the index of the first 🐝
                #     first_bee_idx = df[df[bee_col] == "🐝"].index.min()

                #     # If a bee exists, fill 🍯 after it
                #     if pd.notna(first_bee_idx):
                #         df.loc[first_bee_idx + 1 :, honey_col] = "🍯"

                #     return df


                # intraday = add_bbw_honey_after_bee(intraday)


                # def add_inverted_honey_after_bee(df,
                #                                 bee_col="BBW_Tight_Emoji",
                #                                 honey_col="BBW_Honey_Emoji",
                #                                 f_col="F_numeric",
                ...
                #         current_bias = "above" if current_f > current_td else "below"
                ...
                #         # Look for a cross in opposite direction
                #         if current_bias == "above":
                #             fakeout_mask = next_3[f_col] < next_3[supply_col]
                #         else:
                #             fakeout_mask = next_3[f_col] > next_3[supply_col]

                #         if fakeout_mask.any():
                #             cross_idx = fakeout_mask.idxmax()
                #             df.at[cross_idx, honey_col] = "🍯"
                #             break  # Just once
                #     return df
...
#*************************************************************************************************************************************************************************************
                            # ── System 3 (Upgraded): Find TK Crosses AFTER System 2 ─────────────────────────
                        intraday["System 3"] = ""

                        # Ensure TK cross column is already created
                        # If not, use this to create it:
                        ...
                        # Search forward from System 2 bars to assign System 3
                        system2_idx = intraday.index[intraday["System 2"] != ""]

                        for idx in system2_idx:
                            pos = intraday.index.get_loc(idx)
                            fw = intraday.iloc[pos+1 : pos+10]  # Look forward 9 bars max
                            for i in fw.index:
                                emoji = intraday.at[i, "Tenkan_Kijun_Cross"]
                                if emoji in ["🌞", "🌙"]:
                                    intraday.at[i, "System 3"] = emoji
...
#*************************************************************************************************************************************************************************************
      ...
      # --------------------------------------------------------
                ...
                st.dataframe(
                    intraday.loc[
                        intraday["System 1"] != "",
                        ["Time", "Volume Booster Pre","Volatility Booster Pre", "System 1", "System 2", "Volume Booster Post","Volatility Booster Post","System 3","System 4"]
